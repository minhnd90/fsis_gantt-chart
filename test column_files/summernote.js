    ...
    var hToolbarAndPopoverClick = function (event) {
      ...
      if ($btn.length > 0) {
        ...
        if ($.inArray(sEvent, ['backColor', 'foreColor']) !== -1) {
          ...
          $editable.focus();
          //Fix ui dialog content hidden editor fullscreen
          if($editor.parents(".ui-dialog")){
            $editor.parents(".ui-dialog").addClass("editor_fullscreen");
          }
          ...
          //Fix change ui dialog height, ui dialog position 
          if($editor.parents(".ui-dialog")){
            var og_editor_width = $editor.width();
            var og_editor_height = $editor.height();
            if($btn.hasClass("active")){
              $editor.next(".temp_editor").remove();
              $editor.parents(".ui-dialog").removeClass("editor_fullscreen");
            }else{
              $editor.after('<div class="temp_editor" style="width:'+og_editor_width+'px;height:'+og_editor_height+'px"></div>');
              $editor.parents(".ui-dialog").addClass("editor_fullscreen");
            }
          }
          ...
          if (isFullscreen) {
            ...
            //Fix close editor fullscreen show window scrollbar before close ui dialog
            if(!$editor.parents(".ui-dialog")){
